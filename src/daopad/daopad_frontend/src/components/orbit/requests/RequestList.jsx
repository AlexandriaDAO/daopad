...
// Memoized RequestCard component for performance
const RequestCard = memo(function RequestCard({ request, canApprove, onOpenRequest }) {
  const statusInfo = statusConfig[request.status] || statusConfig.pending;
  const StatusIcon = statusInfo.icon;
  const approvalProgress = request.approval_threshold > 0
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {request.status === 'pending' && canApprove && !hasUserApproved && (
                <Badge variant="outline" className="text-xs">
                  Action needed
                </Badge>
              )}
  ...
  return (
    ...
    prevProps.request.approvals?.length === nextProps.request.approvals?.length &&
  ...
  return (
    <div className="space-y-2">
      {requests.map((request) => {
        const requestCanApprove = canApprove(request);

        return (
          <RequestCard
            key={request.id}
            request={request}
            canApprove={requestCanApprove}
