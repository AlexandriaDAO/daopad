...
pub fn handle_get_info() -> HttpResponse {
    create_json_response(
        ...
        serde_json::json!({
            "status": "ok",
            "message": "Cenup Backend API",
            "version": "1.0.0",
            "canister_id": "vevet-2yaaa-aaaau-abyqa-cai",
            "endpoints": {
                "GET /": "Returns this info message",
                "POST /": "Processes JSON data and logs to console",
                "GET /health": "Health check endpoint",
                "POST /deposit": "Process user deposits (requires authentication)",
            }
        }),
        true,
    )
